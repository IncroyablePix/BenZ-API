...
function create_token(TokenData $data, int $duration_in_minutes): string
{
    ...
    $token_payload = array(
        "iss" => getenv("JWT_ISSUER"),                // Issuer
        "aud" => getenv("JWT_AUDIENCE"),              // Audience
        "nbf" => $now,                                      // Not before
        "iat" => $now,                                      // Issued at
    ...
    return JWT::encode($token_payload, getenv("JWT_PRIVATE_KEY"), "HS256");
}
...
function extract_token_data(string $token): TokenData
{
    try
    {
        $decoded = JWT::decode($token, new Key(getenv("JWT_PRIVATE_KEY"), "HS256"));
    }
    ...
    if($decoded->iss !== getenv("JWT_ISSUER") ||
